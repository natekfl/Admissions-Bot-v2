...
function getTrainerReportGlobal(days) {
    ...
    const embed = {
        ...
        "fields": [
            ...
            {
                "name": "Testing Rates",
                "value": `LLs have a passing rate of ${Math.round(passes / (passes + fails))}% in the selected time period.`
            },
            {
                "name": "Score",
...
function getTrainerReport(user, days) {
    ...
    let totalApprovalsAverage = (totalApprovals / Object.keys(approvers).length);
    ...
    const embed = {
        ...
        "fields": [
            ...
            {
                "name": "Testing Rates",
                "value": `${user.displayName}'s LLs pass the Apprentice test ${Math.round(passesUser / (failsUser + passesUser))}% of the time in the selected time period. This is ${(Math.round(passesUser / (failsUser + passesUser)) > Math.round(passes / (passes + fails))) ? ("above the average") : ("below the average")}, which is ${Math.round(passes / fails)}%`
            },
            {
                "name": "Score",
...
function getTesterReport(user, days) {
    ...
    for (let testId in aTestData) {
        ...
        if (test.time > startEpoch) {
            totalTests += 1;
            testers[test.approvedBy] = true
        }
    }
    let totalTestsAverage = (totalTests / Object.keys(testers).length);
    ...
    let passRate = (passes / (passes + fails)) * 100;
    ...
    const embed = {
        ...
        "fields": [
            ...
            {
                "name": "Testing Rates",
                "value": `${user.displayName}'s LLs pass the Apprentice test ${passRateUser}% of the time in the selected time period. This is ${(passRateUser > passRate) ? ("above the average") : ("below the average")}, which is ${passRate}%`
            },
            {
                "name": "Score",
                "value": `${user.displayName}'s LLs recieve an average score of ${avgTestScoresUser} on the Appprentice Test. This is ${(avgTestScoresUser > avgTestScores) ? ("above the average") : ("below the average")}, which is ${avgTestScores}.`
