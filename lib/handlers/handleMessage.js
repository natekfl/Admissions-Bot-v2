...
let commands = {
    ...
    "!dotrainings": {
        ...
        "description": "Spams the specified users to do trainings. The users tagged must be Senior Inspectors. Has an 8 minute cooldown.",
        "perm": 7,
        "func": function (message) {
            let args = message.content.split(" ");
            if (message.mentions.members.size < 1) return false;
			...
			if (allAreSeniors) {
				cooldowns["!dotrainings"] = true;
				setTimeout(function () {
						cooldowns["!dotrainings"] = false;
				}, (i * 120000))
