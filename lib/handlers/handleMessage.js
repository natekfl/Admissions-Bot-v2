            ...
            if (tools.getPermLevel(subject) === 2) {
                if (subject.id in trialFailData) {
                    if (trialFailData[subject.id].trialsFailed >= 2) {
                        ...
                        message.reply(`${subject.displayName} should not be trialed, they have failed a trial ${Math.round(((new Date).getTime() - trialFailData[subject.id].lastTrialTime) / 43200000)} hours ago.`);
                    } else {
