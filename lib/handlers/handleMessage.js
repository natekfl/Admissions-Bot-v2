                ...
                if (robloxId) {
                    robloxActions.getRankName(robloxId, function (rankName) {
                        if (globalFuncs.setDiscordRankAdmissions(message.member, rankName)) {
                            message.member.setNickname(robloxActions.getUsernameFromId(robloxId));
...
module.exports = function (message) {
    ...
    } else if (message.channel.id === idvariables.channels.rolereqchannel) {
        setTimeout(() => {message.delete()}, 120000) // Delete messages in #role-requests after 2 minutes.
    }
};
