    ...
    processNewReport: function (report) {
        ...
        if (lines.length >= 3) {
            ...
            if (subjectFailCount >= 2) {
                reportError(report, `Subject has failed ${subjectFailCount} Inspector Trials`);
                return false; //End function
            }
            ...
            trainerUserData['approvedTests'][testId] = true;
            ...
            report.channel.send(`Test ID: ${testId} | Approval ID: ${approvalId}`);

            let embed = {
                title: "Inspector Trial " + testId,
                color: 5175215,
                fields: [
                    {
                        name: "User Tested",
            ...
            let messageToDM = (`${report.member.displayName} has submitted your Inspector Trial report. The results are below for your reference.`);

            if (outcome === true) {
                globalFuncs.setDiscordRankAdmissions(subject, "Inspector in Training");
                report.react("✔"); //:heavy_check_mark:

                robloxActions.getRobloxIdFromDiscordID(subject.id, function (robloxId) {
                    if (robloxId) {
                        robloxActions.changeRank(robloxId, "Inspector in Training");
                        report.react("✅"); //:white_check_mark:
                        messageToDM += "You have passed your Inspector Trial, and your ROBLOX and Discord ranks have been updated accordingly.";
                        sendResultDM();
                    } else {
                        report.react("❌"); //:x:
                        messageToDM += "You have passed your Inspector Trial, and your Discord rank was updated accordingly. However, we could not change your ROBLOX group rank. Please contact your trainer for assistance.";
                        sendResultDM();
                    }
                });
            } else {
                 if (subjectFailCount < 2) {
                    messageToDM += "You have failed your Inspector Trial. You cannot get tested again for 12 hours. Use this time to study what you missed.";
                    sendResultDM();
                } else {
                    messageToDM += "You have failed your Inspector Trial. As this is your second failure, you cannot be tested again.";
        ...
        if (subject) {
            ...
            robloxActions.getRobloxIdFromDiscordID(subject.id, function (robloxId) {
                if (robloxId) {
                    robloxActions.changeRank(robloxId, "Inspector in Training");
                    subject.createDM()
                        .then(DMchannel => DMchannel.send("Your ROBLOX group rank has been changed to Inspector in Training"));
                    report.react("✅"); //:white_check_mark:
                } else {
                    report.react("❌"); //:x:
                }
