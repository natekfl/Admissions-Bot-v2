        ...
        if (lines.length >= 3) {
            ...
            if (subject.id in trialFailData) {
                if (trialFailData[subject.id].trialsFailed >= 2) {
                    reportError(report, `Subject has failed ${trialFailData[subject.id].trialsFailed} trials`);
                    return false; //End function
                    ...
                    reportError(report, `Subject has failed a trial ${Math.round(((new Date).getTime() - trialFailData[subject.id].lastTrialTime) / 43200000)} hours ago`);
                    return false; //End function
                }
            }
            ...
            let messageToDM = (`${report.member.displayName} has submitted your trial report. The results are below for your reference.\n\n${report.content}\n\n`);
            ...
            if (outcome===true) {
                ...
                robloxActions.getRobloxIdFromDiscordID(subject.id, function (robloxId) {
                    if (robloxId) {
                        ...
                        messageToDM += "You have passed your trial, and your ROBLOX and Discord ranks have been updated accordingly.";
                        sendResultDM();
                    } else {
                        report.react("❌"); //:x:
                        messageToDM += "You have passed your trial, and your Discord rank was updated accordingly. However, we could not change your ROBLOX group rank. Please contact your trainer for assistance.";
                        sendResultDM();
                    }
                });
            } else {
                ...
                trialFailData[subject.id].lastTrialTime = (new Date).getTime();
                if (trialFailData[subject.id].trialsFailed < 2) {
                    messageToDM += "You have failed your trial. You cannot be trialed again for 12 hours. Use this time to study what you missed."
                    sendResultDM();
                } else {
                    messageToDM += "You have failed your trial. As this is your second failure, you cannot be trialed again."
                    sendResultDM();
                }
                //write to file
...
function reportError(report, error) {
    report.reply(`Your report has errors in it. ${(error) ? ("(" + error + ")") : "Please check that you are using the correct template."} *Your report will be deleted in 10 seconds.*`)
        .then(sentMessage => {
           setTimeout(function () {
               sentMessage.delete();
               report.delete();
